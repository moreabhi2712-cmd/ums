  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4">
        <Field label="Signature (Full Name)" required>
          <input className={inputCls} placeholder="Type your full name as signature"
            value={declarationData.signature}
            onChange={e => updateDeclarationData({ signature: e.target.value })} />
        </Field>
        <Field label="Date" required>
          <input type="date" className={inputCls}
            value={declarationData.date}
            onChange={e => updateDeclarationData({ date: e.target.value })} />
        </Field>
      </div>
    </div>
  );
